...
SELECT [location], [date], [total_cases], [total_deaths], [population], (CAST([total_deaths] As bigint)/CAST([total_cases] As bigint))*100 As DeathPercentage
FROM [CovidDeath]
WHERE continent is not null

SELECT [location], [date], [population], CAST([total_deaths] As bigint) AS TotalDeaths, CAST([total_cases] As bigint) As TotalCases, 
...
SELECT [location], [date], CAST([population] As bigint) As TotalPopulation, CAST([total_cases] As bigint) As TotalCases,
...
SELECT location, population, MAX(CAST(total_cases As float)) As HighestInfectionCount, MAX(CAST([total_cases] As float)/CAST([population] As float))*100 As PercentageCasesByPopulation
FROM [CovidDeath]
WHERE continent is not null
...
SELECT location, population, MAX(CAST(total_deaths As float)) As HighestDeathCount, MAX(CAST([total_deaths] As float)/CAST([population] As float))*100 As PercentageDeathByPopulation
FROM [CovidDeath]
WHERE continent is not null
GROUP BY location, population
ORDER  BY 4 DESC
...
SELECT location, population, SUM(CAST(total_deaths As float)) As HighestDeathCount, MAX(CAST([total_deaths] As float)/CAST([population] As float))*100 As PercentageDeathByPopulation
FROM [CovidDeath]
WHERE continent is not null
GROUP BY location, population
ORDER  BY 3 DESC
